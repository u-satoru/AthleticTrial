# メモ

  進捗状況（2025-12-30時点）

  - Phase 3: キャラクター統合 - 進捗 91%
  - BP_ThirdPersonCharacter: 全13項目実装完了
  - AC_Climbing: 主要16項目実装完了

  未実装タスク（4件）

  1. TryGrabWall - クライミング開始トリガー
  2. ReleaseFromWall - 終了トリガー
  3. UpdateAnimationParameters
  4. UpdateLedgeAnimationParameters

- WallOffset: 40.0
- LocationInterpSpeed: 10.0
- TraceLength: 150.0
- ClimbSpeed: 200.0
- RotationInterpSpeed: 10.0


- キャラクターが壁に吸着するか: 吸着しません。
- 床を通り抜けないか: 通り抜けます。
- WASD で壁を登れるか: 落下するので壁を登れません。
キャラクターの挙動を見ていると、一度壁に取り付いてから、弾き飛ばされているように見えます。

- 壁に吸着するか: 吸着しません。
- 床を通り抜けないか: 床を通り抜けます。
- 弾き飛ばされないか: 「弾き飛ばされる」という表現が的確なのかには自信がありません。ジャンプの途中で、一旦クライミングポーズになってから、更に上昇して、ジャンプの頂点に達した後に落下しています。


- 壁に吸着するか: Spaceキーを押し続ければ吸着します。
- 上昇し続けないか: 上昇しなくなりました。
- WASD で壁を登れるか: 登れません。

1. HitWall: true
2. Condition Result: true

- WallJump Started が表示されない: 表示されない
- 壁に吸着する: 吸着する
- WASD で壁を登れる: 登れない。反応無し

1. 壁の近くでスペースキーを押して壁に吸着: 壁に吸着する
2. WASD キーで壁を上下左右に移動できるか確認: WASD キー のどれを押しても右移動になる。また壁を超えてもそのまま移動し続ける


  | キー | X (Horizontal) | Y (Vertical) |
  |------|----------------|--------------|
  | W    | 0              | 0            |
  | S    | 0              | 0           |
  | A    | 0             | 0            |
  | D    | 0              | 0            |

  | キー | 表示される値 |
  |------|--------------|
  | W    | X=0, Y=1     |
  | S    | X=1, Y=0    |
  | A    | X=1, Y=0    |
  | D    | X=1, Y=0     |


  | キー | Action Value X | Action Value Y |
  |------|----------------|----------------|
  | W    | 0              | 1              |
  | S    | 1              | 0              |
  | A    | 1              | 0              |
  | D    | 1              | 0              |

- W（前進）: 動く
- S（後退）: 動く
- A（左移動）: 動く
- D（右移動）: 動く


1. Make Vector 2D の X ピンのワイヤーをクリック → Action Value X
2. デバッグ Print String の入力ワイヤーをクリック → 1つ目の Action Value X、2つ目の Action Value Y
3. 両方が同じ IA_Move ノードか確認 → 両方が同じ


  | 順番 | キー | Action Value X | Action Value Y |
  |------|------|----------------|----------------|
  | 1    | W    | 0              | 1              |
  | 2    | A    | 1              | 0              |
  | 3    | S    | 1              | 0              |



1. Action Value X
2. Action Value Y


  | キー | 期待する動作 | 確認結果 |
  |------|--------------|----------|
  | W    | 上に移動     | X=0, Y=1  |
  | S    | 下に移動     | X=1, Y=0        |
  | A    | 左に移動     | X=1, Y=0        |
  | D    | 右に移動     | X=1, Y=0        |

  1. ASDで移動する
  2. 上下は止まらずに上移動の場合は、下移動に変わる。。下移動の場合は上移動に変わる。 左右は壁を超えて移動する止まらない。
     1. 腰のあたりから法線が出ているので、上移動の場合は、腰の位置まで移動してから下移動に変わる。



  3. 壁の端（左右・上下）に到達した時：
    - 壁を超えて移動し続けるか？ → 止まるべき
    - 壁の表面に沿って移動できるか？
  

- WASD を押している間だけ移動するか → 移動する
- キーを離すと停止するか → 停止しない

1. 移動する。
2. 移動は停止するが、キャラクターが壁に吸着した位置に移動する。
   1. 再度、同じ移動キーを押すと、以前移動した位置から移動する


  1. 壁に吸着して WASD で移動: 移動する
  2. キーを離した時：
    - キャラクターが移動した位置で停止するか？: 移動した位置で停止しない
    - 元の吸着位置に戻らないか？: 吸着した位置に戻る



1. 壁に吸着: 吸着する
2. WASD で移動（WallHitLocatio の値が変化するか確認）: 移動はするが、 **WallHitLocatio の値が変化しない**
3. キーを離す
4. Output Log で：
 - 「No input - skipping Line Trace」が表示されるか: 表示される
 - WallHitLocatio の値がキーを離した後も維持されているか: 維持はされているが、 **WASD で移動時に WallHitLocatio の値が変化していない**



@Content\FreeAnimationLibrary\Animations\Climbing_Animations\Climbing\ のアニメーションの Enable Root Motion に チェックを入れましたが問題ありませんか？

@Content\FreeAnimationLibrary\Animations\Climbing_Animations\Climbing\anim_Climb_Down_Right.uasset
@Content\FreeAnimationLibrary\Animations\Climbing_Animations\Climbing\anim_Climb_Down.uasset
@Content\FreeAnimationLibrary\Animations\Climbing_Animations\Climbing\anim_Climb_Left.uasset
@Content\FreeAnimationLibrary\Animations\Climbing_Animations\Climbing\anim_Climb_Right.uasset
@Content\FreeAnimationLibrary\Animations\Climbing_Animations\Climbing\anim_Climb_Up_Left.uasset
@Content\FreeAnimationLibrary\Animations\Climbing_Animations\Climbing\anim_Climb_Up_Right.uasset
@Content\FreeAnimationLibrary\Animations\Climbing_Animations\Climbing\anim_Climb_Up.uasset

上記の Enable Root Motion に チェックを入れましたが問題ありませんか？


- Line Trace HIT が表示されない
- WASD 移動時に Line Trace MISSED が表示される
- WASD 移動時は Input detected - doing Line Trace が表示される 

  1. 壁に張り付く: 壁に吸着する
  2. WASD で移動: 移動する。 **ただしCapsuleが移動しない**
  3. Line Trace HIT が表示されるか確認: Line Trace HIT は表示されず、WASD 移動時に Line Trace MISSED が表示される
  4. キャラクターが壁に沿って移動するか確認 : 壁を超えて移動する

  | 確認項目                         | 期待結果 |
  |----------------------------------|----------|
Line Trace HIT が表示される: **壁吸着時には表示されない** が、WASD 移動時に表示される
キャラクターが壁に沿って移動する: 移動する
壁を超えて移動しない: 壁を超えて移動する **ただし壁を超えたときには、Capsule の移動は停止する**
Capsule も一緒に移動する: **Capsule も一緒に移動するが、Mesh の移動より遅い**


  | 設定項目                                | 現在の値 |
  |-----------------------------------------|----------|
  | ABP の Root Motion Mode                 | No Root Motion Extraction        |
  | LocationInterpSpeed                     | 100.0        |
  | BP_ThirdPersonCharacter → Mesh            | Root Motion Mode | 見当たらない        |



S キー（下降）: 黄色の Line Trace が下方向に表示           |
床に近づく: クライミングが終了（ReleaseFromWall）するが、 **床を通り抜けて落下する**
W キー（上昇）: 赤い Line Trace が壁方向に表示（黄色なし）
A/D キー（水平）: 赤い Line Trace が壁方向に表示（黄色なし）


Asset Details パネルで以下を確認：
- Enable Root Motion にチェックが入っているか: チェックが入っている
- Root Motion Root Lock の設定: Ref Pose
- Force Root Lock の設定: OFF
1. Enable Root Motion の状態: ON
2. Force Root Lock の状態: Ref Pose
3. Root Motion Root Lock の値: OFF

- W キーを押したとき: X = 0, Y = 1
- A キーを押したとき: X = 1, Y = 0
- S キーを押したとき: X = 1, Y = 0
- D キーを押したとき: X = 1, Y = 0


- IsAtClimbTop= false
- bIsClimbing= true


期待される表示メッセージ
壁の端に到達したとき（Line Trace MISSED 後）:

状況	期待されるメッセージ
壁の端に到達	Line Trace MISSED
内側コーナーチェック開始	CheckInnerCorner: Called
内側コーナー検出	CheckInnerCorner: HIT - Inner Corner Found
内側コーナー未検出	CheckInnerCorner: MISS - No Inner Corner
外側コーナーチェック開始	Hello（CheckOuterCorner: Called 推奨）
外側コーナー検出	CheckOuterCorner: No Wall - Outer Corner Found!
外側コーナー未検出	CheckOuterCorner: Wall Found - NOT Outer Corner
**再度、動画全てを分析を実施して、どのメッセージが表示されるか教えてください。**

今度は表示されるはずのメッセージ
状況	期待されるメッセージ
壁の端に到達	Line Trace MISSED
内側コーナーチェック開始	CheckInnerCorner: Called ← 今度は表示されるはず
内側コーナー検出	CheckInnerCorner: HIT - Inner Corner Found
内側コーナー未検出	CheckInnerCorner: MISS - No Inner Corner
外側コーナーチェック開始	Hello
外側コーナー検出	CheckOuterCorner: No Wall - Outer Corner Found!
外側コーナー未検出	CheckOuterCorner: Wall Found - NOT Outer Corner
**動画全てを分析を実施して、どのメッセージが表示されるか教えてください。**
動画ファイル（`2026-01-17 09-55-59.mp4`）を分析しました。

今回は、前回表示されていなかったデバッグメッセージ（`Hello`や`CheckOuterCorner...`など）が明確に確認できました。ただし、**「外側コーナー（凸）」と「内側コーナー（凹）」で結果が大きく異なります。**

以下に、キャラクターの動作別に表示されたメッセージを一覧にします。

---

### デバッグログ表示分析結果

#### 1. 外壁・右移動時のコーナー検知（00:21 付近）

キャラクターが壁の右端（凸コーナー）に到達した際、一瞬で以下のメッセージログが積み上がりました。
画面上の**上から下（新しい順）**の表示は以下の通りです。

| 表示順 | メッセージ色 | メッセージ内容 | 意味・分析 |
| --- | --- | --- | --- |
| **1 (最新)** | **緑** | **CheckOuterCorner: No Wall - Outer Corner Found!** | **外側コーナー検知成功！** |
| **2** | **青** | **Hello** | 外側チェック開始 (`CheckOuterCorner` 開始) |
| **3** | **黄** | **CheckInnerCorner: MISS - No Inner Corner** | 内側コーナーは見つからなかった |
| **4** | **緑** | **CheckInnerCorner: Called** | 内側チェック開始 |
| **5 (最初)** | **赤** | **Line Trace MISSED** | 壁がなくなった（トリガー） |

* **動作:** この直後、コーナー移動処理がないためキャラクターは落下します。
* **結論:** 外側コーナー（Outer Corner）のロジックは完璧に動作し、全てのメッセージが期待通り表示されています。

---

#### 2. 外壁・左移動時のコーナー検知（00:36 付近）

左側の壁端に到達した際も、右移動と同様に正常に検知されています。

| 表示順 | メッセージ色 | メッセージ内容 |
| --- | --- | --- |
| **1 (最新)** | **緑** | **CheckOuterCorner: No Wall - Outer Corner Found!** |
| **2** | **青** | **Hello** |
| **3** | **黄** | **CheckInnerCorner: MISS - No Inner Corner** |
| **4** | **緑** | **CheckInnerCorner: Called** |
| **5 (最初)** | **赤** | **Line Trace MISSED** |

---

#### 3. 内壁・右移動（00:46 付近）および 左移動（00:55 付近）

**ここでは期待されるメッセージが表示されませんでした。**
キャラクターが部屋の隅（凹コーナー）に到達しましたが、ログは以下の「壁吸着中」の状態から変化しませんでした。

| 表示順 | メッセージ色 | メッセージ内容 | 状態 |
| --- | --- | --- | --- |
| **維持** | **青/赤** | **Line Trace HIT** | 壁を検出し続けている |

* **なぜ表示されないのか？（重要）**
* 現在のロジック（動画最後のBP参照）では、`Line Trace` が **MISSED（失敗）** しないと、コーナー検知処理（`CheckInnerCorner`）へ分岐しません。
* 内側コーナー（凹形状）の場合、進行方向のトレースは「現在登っている壁」から外れても、即座に「隣の壁（直角にある壁）」に **HIT** してしまいます。
* そのため、システムは「まだ壁が続いている」と判断し、`Line Trace MISSED` にならず、コーナー検知ロジックが呼び出されていません。



---

### まとめ

ユーザー様のリクエストに基づき、表示されたメッセージを整理しました。

1. **外側コーナー（凸）:**
* 期待されるメッセージは **全て表示されました**。
* 表示順（画面上から）：`Outer Found!` -> `Hello` -> `Inner Miss` -> `Inner Called` -> `Trace MISSED`


2. **内側コーナー（凹）:**
* 期待されるメッセージは **一つも表示されませんでした**。
* 常に `Line Trace HIT` が表示され続けました。



**次のステップ:**
内側コーナーを検知するためには、`Line Trace HIT` の状態でも、「現在の壁との角度」や「法線（Normal）の変化」を検知して `CheckInnerCorner` を呼び出すようなロジックの追加が必要になると思われます。



添付した動画は内側コーナー移動の動画です。以下を確認して

テスト確認項目
内側コーナーに向かって移動（A または D キー）
「Corner Transition: Distance < Threshold!」 メッセージが表示されるか
キャラクターが次の壁を向いて回転するか
壁から適切な距離（WallOffset = 40）で吸着するか
壁の縁ではなく、適切な位置に移動するか


Step 2: 次の壁を探す
─────────────────────
           Start
             │
    ─────────┐    │
              │←──┘ Line Trace（壁Aの前方 + 移動方向にオフセット）
              │
              │ 壁B にヒット！



添付した画像は外側コーナーを移動するための次の壁の検出をするテストです。
以下を確認：
「Edge detected, searching next wall...」が表示される
「Next wall found! Transitioning...」が表示される
キャラクターが次の壁に回り込む
キャラクターの回転が正しく適用される


[現在の壁]          [次の壁]
    |                  |
    |    Start ------> | ← End（次の壁にヒット）
    |      ↑           |
    |   移動方向       |
    |      |           |
   [キャラクター]

       [次の壁]
           |
           |  ← 2nd Trace（壁にヒット）
           |
    Start2 ●----→ End2
           ↑
           | 1st Trace（壁が無い = 端を検出）
           |
    Start1 ●----→ End1 (MISS)
           |
    [現在の壁]
           |
     [キャラクター]

    [現在の壁]        [次の壁]
         |               |
         |               |
         |    2nd End    |
         |       ↓       |
         |    ●←←←←←←●   |
         |   2nd Start   |  ← 2nd Trace（移動方向の反対へ、次の壁にヒット）
         |       ↑       |
         |  OuterCornerOffset
         |       ↑       |
    End1 ●----→ 1st (MISS) ← 1st Trace（移動方向へ、端を検出）
         |               
    Start1（キャラクター位置）
         |


**設計が間違っています**
再度、文字で示します。
- 1st Trace Line
  - 1st Trace Line の Start は **キャラクター移動方向へ数十センチオフセットした位置**
  - 1st Trace Line の End は Start からキャラクターの前方方向へ数十センチ
- 2nd Trace Line
  - 2nd Trace Line の Start 1st Trace Line の End
  - 2nd Trace Line の End は Start から移動方向の反対へ数十センチ

```
               [現在の壁]
                   |
                   | 
                   | 
                   |
     キャラクター ● |
           |       +------●----[次の壁]   
           |       |       ↑
移動方向にオフセット |       |     End（次の壁にヒット） 
           |       | 2nd Tsrace（移動方向の反対へ、次の壁にヒット）
1st Trace(Start)  -------●Start
```

添付した動画は外側コーナー検出のためのテストです。
画像を解析して "MoveDirection:" の文字列が含まれるメッセージを検出して示して

壁移動で外コーナーを検出して、次の壁へ移動するテストです。
動画を分析して文書として表して

処理のフローをを確認します。以下で認識は正しいですか？
1. 現在の壁Aの終端を検出
2. 外側コーナーの次の壁Bを検出するために、キャラクター移動方向へ + オフセットした位置から 1st Trace Start
3. 1st Trace End 位置を 2nd Trace Start にする
4. 2nd Trace End は キャラクター移動方向の反対へ発射


動画を分析して、以下のメッセージが表示されているか確認してください。

1. "Line Trace MISSED" （ピンク色）
2. "CheckOuterCornerByDistance: CalledHello" （オレンジ色）
3. "Outer Corner: Edge detected, searching next wall..." （黄色）
4. "Outer Corner: Next wall found! Transitioning..." （緑色）

動画を分析して、以下のメッセージが表示されているか確認してください。
"Moving to: X=[座標] Y=[座標] Z=[座標]" (紫色)


動画を分析して、以下のメッセージが表示されているか確認してください。表示されていれば、メッセージを示して
1. "Outer Corner: Next wall found! Transitioning..."（緑）
2. "Distance: [実際の距離値]"（白）
3. "MaxCornerDistance: [最大許容距離]"（白）



動画を分析して、以下のメッセージが表示されているか確認してください。表示されていれば、メッセージを示して
1. Outer: Setting Actor Rotation
2. NextWallNormal:


動画を分析して、以下のメッセージが表示されているか確認してください。表示されていれば、メッセージを示して
- 「Outer Corner: Edge detected, searching next wall...」（黄色）- 1st Trace MISS
- 「Outer Corner: Next wall found! Transitioning...」（緑）- 2nd Trace HIT
- 「Distance: [数値]」と「MaxCornerDistance: [数値]」（白）- 距離チェック




# CLAUDE.md

**Ultra Think**

Unity 6で作成された3Dゲームプロジェクトのベーステンプレートです。Universal Render Pipeline (URP) を使用しています。

## 概要

*Service Locator + Event駆動のハイブリッドアーキテクチャ（最重要）*とコマンドパターンを組み合わせた、拡張性の高い3Dゲームの基盤を提供します。Scriptable Objectを活用し、データとロジックの分離を促進することで、効率的な開発とメンテナンスを目的としています。

## プロジェクトステータス ✅完了 (2025-12-28)

| 指標 | 達成値 |
|------|--------|
| コンパイルエラー | 378→0件 (100%解決) |
| コンパイル警告 | 0件 |
| ServiceLocator統合 | 1,265箇所 |
| Singleton排除 | 524→0箇所 (100%達成) |
| GameEvent定義 | 150個 |
| ICommand実装 | 188個 |
| Assembly Definition | 39個 |
| パフォーマンス | 95%メモリ削減、67%速度改善 |
| 文字エンコーディング | UTF-8統一、LF改行 |

### DOTS/ECS ✅導入完了
- 導入可能性評価: 90%以上実現可能
- 推奨アプローチ: ハイブリッドMonoBehaviour + ECS
- 期待効果: AIセンサー処理300-500%向上、メモリ70%削減
- 実装期間: 6-8ヶ月（3フェーズ）
- 詳細: [`Docs/07_Reports/DOTS_ECS_Introduction_Feasibility_Report.md`](Docs/07_Reports/DOTS_ECS_Introduction_Feasibility_Report.md)

## 技術仕様

| 項目 | 値 |
|------|-----|
| Unity Version | **6000.3.2f1** |
| Unity Editor Path | `C:\Program Files\Unity\Hub\Editor\6000.3.2f1\Editor\Unity.exe` |
| Render Pipeline | Universal Render Pipeline (URP) |
| Scripting Backend | Mono |
| API Compatibility | .NET Standard 2.1 |

## アーキテクチャ制約

- **Singletonパターンは原則禁止**（ServiceLocatorで代替）
- **JSON操作にはNewtonsoft Jsonを使用**（JsonUtility禁止）

## 4層アーキテクチャ（最重要）

| 層 | ディレクトリ | 編集 | 説明 |
|----|-------------|------|------|
| Core | `Assets/_Project/Core` | 🔒禁止 | ジャンル非依存の普遍的な仕組み（ServiceLocator、Event、Command、HSM） |
| Feature | `Assets/_Project/Features` | 🔒禁止 | 再利用可能な機能部品（Player、AI、Camera、Combat） |
| Template | `Assets/_Project/Templates` | 📖参照・コピー | ゲームジャンルのひな形（シーン、プレハブ、ScriptableObject） |
| **Game** | `Assets/_Project/Game` | ✏️**自由** | **ゲーム固有実装。ユーザーはここで開発** |

**依存方向**: Game → Template → Feature → Core（逆方向禁止、39個のasmdefで強制）

## アーキテクチャとデザインパターン

### 1. Service Locator + Event駆動のハイブリッドアプローチ（最重要）
- `ServiceLocator`を通じて`AudioManager`や`InputManager`などのグローバルサービスにアクセス
- `GameEvent`を通じて各層が疎結合に連携
- 依存関係の注入が不要で、コードの可読性と保守性が向上

### 2. イベント駆動型アーキテクチャ
- `ScriptableObject`ベースのイベントチャネル（`GameEvent`）使用
- オブジェクト間の直接参照なしで通信

### 3. コマンドパターン + ObjectPool最適化
- ゲーム内操作を「コマンド」としてカプセル化
- Undo/Redo、遅延実行、キューイングが容易
- `CommandPool`で95%メモリ削減、67%速度改善

### 4. 階層化ステートマシン (HSM)
- Player、Camera、AIの複雑な状態管理
- 状態のネストとロジックの継承が可能

### 5. ScriptableObjectベースのデータ駆動設計
- キャラクターステータス、アイテムスペック等をアセットとして管理
- ノンプログラマーもバランス調整可能

### 6. その他のパターン
- **Strategyパターン**: アルゴリズムのカプセル化と交換
- **Factory+Registryパターン**: オブジェクト生成と管理の分離
- **Lifecycle Managementパターン**: オブジェクトの生成・使用・破棄を一元管理
- **Delegateパターン**: コールバックとイベントハンドリング

## 制作想定ゲーム

**シングルプレイ専用**:
- ステルスアクションゲーム
- サバイバルホラーゲーム
- 3Dプラットフォーマーゲーム
- TPS/FPSゲーム（一人称/三人称視点切り替え可能）
- アクションRPGゲーム
- アクションアドベンチャーゲーム

**対応プラットフォーム**: Windows、iOS、Android

## 主要パッケージ

| パッケージ | 用途 |
|-----------|------|
| `com.unity.render-pipelines.universal` | Universal Render Pipeline |
| `com.unity.inputsystem` | 入力システム（**旧Input Manager API禁止**） |
| `com.unity.cinemachine` | 高度なカメラ制御 |
| `com.unity.ai.navigation` | ナビゲーションと経路探索 |
| `com.unity.test-framework` | ユニットテストとプレイモードテスト |
| `com.unity.timeline` | タイムラインとカットシーン制御 |
| `com.unity.addressables` | アセット管理と動的ロード |
| `com.unity.probuilder` | 3Dモデリングとレベルデザイン |
| `com.unity.localization` | 多言語対応・ローカライゼーション |

### UI システム
- **uGUI** (現在のランタイムUI): Canvas + DOTween統合済み
- **UIToolkit** (利用可能): `com.unity.modules.uielements` 導入済み
  - エディタUI拡張に推奨
  - ランタイムUIは段階的移行可能
  - .uxml/.uss によるスタイル一元管理
- **TextMesh Pro**: 高品質テキストレンダリング（標準統合）

### DOTS/ECSパッケージ（導入済み）
- `com.unity.entities`: Entity Component System基盤
- `com.unity.entities.graphics`: ECSレンダリング
- `com.unity.burst`: Burstコンパイラ（高速化）
- `com.unity.collections`: ネイティブコレクション
- `com.unity.jobs`: Job System（並列処理）
- `com.unity.mathematics`: 数学ライブラリ

### 非同期処理
- **Unity 6 Awaitable** (必須): Unity公式のゼロGC async/await API
- ✅ UniTask → Unity 6 Awaitable 移行完了（TASK-013, 2026-01-01）

### サードパーティ
- **DOTween**: アニメーションとタイムライン制御
- **Tri Inspector**: エディタ拡張とカスタムインスペクタ（MIT License）
  - ✅ Odin Inspector から移行完了（2026-01-03）
- **Newtonsoft Json** (`com.unity.nuget.newtonsoft-json`): JSON操作（**必須使用**）
  - ✅ JsonUtility から移行推奨
- **FluidBT** (`com.fluid.behavior-tree@2.3.0`): 純粋C# Behavior Tree
  - **Prototype層のAI Behavior実装で使用**（BD Proはビジュアル設計用）
  - NPM Registry経由でインストール済み

## 開発環境要件

- **CLI**: PowerShell 7を使用（cmd非推奨）
- **タイムゾーン**: JST (UTC+9)
- **日付形式**: `YYYYMMDD_HHMM`（例: `20231005_1430`）
- **文字エンコーディング**: UTF-8
- **改行コード**: LF
- **コードスタイル**: [Unity C# Coding Standards](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/coding-style/coding-conventions) に準拠
- **Git LFS**: 大きなバイナリファイルに使用

### Git操作規約

| 操作 | コマンド | 理由 |
|------|----------|------|
| ファイル/ディレクトリ移動 | `git mv` | Git履歴の追跡を維持 |
| ファイル/ディレクトリリネーム | `git mv` | Git履歴の追跡を維持 |

```bash
# 例: ディレクトリのリネーム
git mv Docs/OldName Docs/NewName

# 例: ファイルの移動
git mv path/to/file.md new/path/file.md
```

**禁止**: `Move-Item`、`Rename-Item`、エクスプローラーでの直接操作（Git履歴が途切れる）

### ⚠️ 最重要制約

> **【最重要】バッチモード（コンパイル・テスト実行等）を起動する前には、必ず Unity Editor が起動していないかプロセスを確認し、起動している場合は終了してください。**
> 同一プロジェクトへの複数プロセス同時アクセスはファイル破損の原因となります。

### Windows PowerShellコマンド例
```powershell
# ファイル操作はpowershell -Commandでラップ
powershell -Command "Remove-Item 'パス'"
powershell -Command "Move-Item 'ソース' 'デスティネーション'"
powershell -Command "New-Item -ItemType Directory -Path 'パス'"
```

## ディレクトリ構成

```
Assets/_Project/
├── Core/           # 🔒 ServiceLocator、Events、Commands、ECS
├── Features/       # 🔒 Player、AI、Camera、Combat
├── Templates/      # 📖 ジャンル別テンプレート
│   ├── [ジャンル名]/   # Stealth, SurvivalHorror, FPS, TPS等
│   └── Common/         # 共有アセット
├── Game/           # ✏️ ゲーム固有実装
│   ├── Scripts/        # ゲーム固有ロジック
│   ├── Prefabs/        # ゲーム固有プレハブ
│   ├── Scenes/         # ゲームシーン
│   ├── Data/           # ScriptableObject設定
│   └── Animations/     # ゲーム固有アニメーション
├── Tests/          # テストコード
├── ThirdParty/     # サードパーティラッピング
└── Sandbox/        # 個人実験用（Git除外）
Docs/               # ドキュメント
Docs/99_Works/      # 作業報告（YYYYMMDD_HHMM形式）
Logs/               # ログファイル（Assets外に配置）
Assets/Plugins/     # サードパーティ（直接編集禁止）
```

### 開発・テスト環境

| 環境 | ディレクトリ | Git | 用途 |
|------|-------------|-----|------|
| Feature層リファレンス | `Templates/Common/Sandbox/` | ✅ | Feature層機能のリファレンス |
| ゲーム固有開発 | `Game/Development/` | ✅ | ゲーム固有機能の開発確認 |
| 個人Sandbox | `Assets/_Project/Sandbox/` | ❌ | 個人実験・デバッグ（コミット不要） |

**ローカル専用シーン**: `*_local.unity`（.gitignore除外）

### Assembly Definition構成（39個）

| 層 | 数 | 主要名 |
|----|---|--------|
| Core | 9 | `asterivo.kiban.Core.*` |
| Feature | 14 | `asterivo.kiban.Features.*` |
| Template | 7 | `asterivo.kiban.Templates.*` |
| Game | 1 | `asterivo.kiban.Game` |
| Tests | 7 | `asterivo.kiban.Tests.*` |
| Editor | 1 | `asterivo.kiban.Editor.*` |

### ECSディレクトリ構造

```
Assets/_Project/Core/ECS/
├── Components/     # ECSコンポーネント定義
├── Systems/        # ECSシステム
├── Authoring/      # MonoBehaviour→ECS変換
├── Jobs/           # Burst対応Job定義
├── Aspects/        # ECS Aspects
└── Utilities/      # ECSユーティリティ
```

**ハイブリッド方針**: ECSはCore層に配置、ServiceLocator経由でMonoBehaviourと連携

## SDD（スペック駆動開発）

| ファイル | 用途 | 文字数制限 |
|----------|------|-----------|
| `CLAUDE.md` | プロジェクト概要・規約（人間作成） | **40,000 chars** |
| `SPEC.md` | 初期構想・要件定義（人間作成） | **40,000 chars** |
| `REQUIREMENTS.md` | 形式化された要件（AI生成） | **40,000 chars** |
| `DESIGN.md` | 技術設計書（AI生成） | **40,000 chars** |
| `TASKS.md` | 実装タスク一覧（AI生成） | - |
| `TODO.md` | 進行中タスク管理（人間作成） | - |

**SDDフェーズ**: 構想 → 形式化 → 設計 → 分解 → 実装・検証

**作業ログ**: `Docs/99_Works/YYYYMMDD_HHMM/`にスナップショット保管

### SDDファイル文字数制限（重要）

| ファイル | 制限 | 運用ルール |
|----------|------|-----------|
| CLAUDE.md | 40,000 chars | 概要・規約のみ、詳細は`Docs/`へ |
| DESIGN.md | 40,000 chars | 設計概要のみ、詳細Specは`Docs/05_Specs/`へ |
| SPEC.md | 40,000 chars | 要件概要のみ、Feature Specは分離 |
| REQUIREMENTS.md | 40,000 chars | 形式化要件のみ |

**超過時の対応**:
- 詳細仕様は `Docs/05_Specs/` に分離
- 基盤ファイルは概要・リンクのみ記載
- 定期的に文字数を確認（`wc -m` or エディタ機能）

### SDD×TDD改善提案

Feature層のテスト駆動開発を強化するための改善提案を策定済み。

| ドキュメント | 説明 |
|-------------|------|
| [`SDDxTDD_アーキテクチャ改善提案.md`](Docs/08_Proposals/SDDxTDD_アーキテクチャ改善提案.md) | 改善提案の統合文書 |
| [`SDDxTDD_改善提案_メリットデメリット分析.md`](Docs/08_Proposals/SDDxTDD_改善提案_メリットデメリット分析.md) | メリット/デメリット分析 |
| [`SDDxTDD_追加ファイル一覧.md`](Docs/99_Works/SDDxTDD_追加ファイル一覧.md) | 追加ファイル一覧 |
| [`SDDxTDD_実装タスク一覧.md`](Docs/99_Works/SDDxTDD_実装タスク一覧.md) | 実装タスク一覧 |

**主要改善ポイント**:
- Service Locator設計ルール（SL-1〜SL-3）
- Command Pool設計ルール（P-1〜P-3）
- Feature Spec形式の標準化
- Feature TDD vs Golden/Replayテスト戦略

詳細: [`Docs/SDD_Markdown作成実践ガイド.md`](Docs/SDD_Markdown作成実践ガイド.md)

## MCPサーバー優先順位

| サーバー | 役割 | 制御 | 主要ユースケース |
|----------|------|------|------------------|
| ddg-search | Web検索 | 読取専用 | 最新情報検索 |
| context7 | 技術文書検索 | 読取専用 | API/ライブラリドキュメント |
| deepwiki | コードベース分析 | 読取専用 | アーキテクチャ理解 |
| github | GitHub API | 読み書き | Issue/PR管理 |
| git | バージョン管理 | 読み書き | コミット・ブランチ管理 |
| blender-mcp | 3D制作 | 読み書き | 3Dアセット生成 |
| ai-game-developer | Unity操作 | 読み書き | レベルデザイン・テスト実行 |

**3Dゲーム制作優先度**: ai-game-developer > context7 > deepwiki > ddg-search > github > git > blender-mcp

**意思決定ルール**:
1. 読み取り専用サーバーを先に使用して情報収集
2. 書き込み操作はユーザー承認後に実行

## Addressables（積極活用）

**目標**: 初期インストール50-80MB、DLC対応

| アセット種別 | 配信可能 | 例 |
|-------------|----------|-----|
| ScriptableObject | ✅ | AIデータ、武器設定 |
| Prefab | ✅ | キャラクター、武器 |
| Scene | ✅ | ステージ、エリア |
| Audio | ✅ | BGM、SE、ボイス |
| Visual | ✅ | テクスチャ、マテリアル |
| C#スクリプト | ❌ | 必ずビルド時に組み込み |
| Assembly Definition | ❌ | 必ずビルド時に組み込み |

**実装ガイドライン**:
- ラベル活用: `stage`, `ai`, `audio`, `dlc`等で分類
- 非同期ロード: `Addressables.LoadAssetAsync<T>()` + Unity 6 Awaitable
- メモリ管理: 未使用アセットの適切な`Release()`

詳細: [`Docs/03_Systems/Addressables/ADDRESSABLES_REMOTE_ASSETS_GUIDE.md`](Docs/03_Systems/Addressables/ADDRESSABLES_REMOTE_ASSETS_GUIDE.md)

## テストガイドライン

### テスト環境

| 環境 | 実行場所 | 特徴 |
|------|----------|------|
| Unity Editor | ローカル(GUI) | 全アセットアクセス可、デバッグ容易 |
| ローカルBatch Mode | ローカル(CLI) | 高速、CI前検証 |
| GitHub Actions CI | クラウド | 自動品質ゲート、Docker環境 |

### テストモード

| モード | 用途 | asmdef設定 |
|--------|------|------------|
| **EditMode** | 純粋C#ロジック、ロジック分離パターン | `"includePlatforms": ["Editor"]` |
| **PlayMode** | MonoBehaviour、Physics、UI依存 | `"includePlatforms": []` |

**EditModeテストを優先**: Unity依存を最小化し、高速で安定したテスト

### 推奨パターン

#### 1. ロジック分離パターン（最重要・推奨）

> **学術名**: Humble Object パターン（xUnit Test Patterns, Gerard Meszaros）
> テスト困難なコード（Unity依存）を「謙虚」に保ち、ロジックを分離する設計パターン

MonoBehaviour / ScriptableObject から純粋C#ロジックを分離し、**本番コード**をEditModeでテスト可能にする。

**適用対象と基準**:

| 対象 | 適用基準 | 例 |
|------|----------|-----|
| MonoBehaviour | ビジネスロジックあり | PlayerController, AIStateMachine |
| ScriptableObject | ビジネスロジックあり | GameEvent, AIBehavior, DamageFormula |
| 純粋データSO | ロジックなし | ❌ 適用不要（CharacterStats等） |

**Core層は優先適用**: GameEvent、Command等のCore層コンポーネントは最優先でロジック分離。

**ディレクトリ構造**:
```
# Feature層
Features/{Feature}/
├── Logic/
│   └── {Component}Logic.cs    # 純粋C#（テスト対象）
└── {Component}.cs              # Humble Object（委譲のみ）

# Core層
Core/{Domain}/
├── Logic/
│   └── {Component}Logic.cs    # 純粋C#（テスト対象）
└── {Component}.cs              # Humble Object（委譲のみ）
```

```csharp
// ❌ 非推奨: Simulatorパターン（テスト内で再実装 → カバレッジ0%）
public class PlayerControllerTests
{
    public class PlayerControllerSimulator { ... }  // 再実装
}

// ✅ 推奨: ロジック分離パターン（本番コードをテスト → カバレッジ計測）
// Production: Features/Player/Logic/PlayerControllerLogic.cs
public class PlayerControllerLogic  // 純粋C#、本番コード
{
    private bool _movementFrozen;
    public bool IsMovementFrozen => _movementFrozen;
    public void FreezeMovement() => _movementFrozen = true;
}

// Test: Tests/Features/Player/PlayerControllerLogicTests.cs
[TestFixture]
[Category("EditMode")]
[Category("Unit")]
public class PlayerControllerLogicTests
{
    private PlayerControllerLogic _logic;

    [SetUp]
    public void SetUp() => _logic = new PlayerControllerLogic();

    [Test]
    public void FreezeMovement_SetsFlag_True()
    {
        _logic.FreezeMovement();
        Assert.IsTrue(_logic.IsMovementFrozen);  // 本番コードをテスト
    }
}
```

**原則**:
- ビジネスロジックは`{Component}Logic.cs`に抽出（本番コード）
- MonoBehaviour / ScriptableObject は入出力の薄いラッパー（Humble Object）
- テストは`{Component}LogicTests.cs`で**本番コード**をテスト
- Core層（GameEvent等）は優先的にロジック分離を適用

#### 2. AAAパターン
Arrange-Act-Assert構造を一貫して使用。

#### 3. テスト命名規約
```
{メソッド名}_{シナリオ/条件}_{期待される結果}
```

| パターン | 例 |
|----------|-----|
| 基本形 | `Execute_WithValidTarget_DealsDamage` |
| When形式 | `Jump_WhenGrounded_Succeeds` |
| Returns形式 | `CanJump_WhenInAir_ReturnsFalse` |
| Throws形式 | `GetService_WhenNotRegistered_ThrowsException` |

#### 4. カテゴリ分類
```csharp
[TestFixture]
[Category("EditMode")]    // 実行環境（必須）
[Category("Unit")]        // テストタイプ（必須）
[Category("Core")]        // レイヤー（推奨）
[Category("Player")]      // 機能領域
public class MyTests { }
```

### 重要な原則

| 原則 | 説明 |
|------|------|
| EditMode優先 | Unity依存を最小化 |
| ロジック分離 | 本番コードでロジック分離し、テストで直接テスト（カバレッジ計測） |
| 本番コードテスト | Simulatorでの再実装ではなく、本番のLogicクラスをテスト |
| 1テスト1検証 | 1つのテストで1つの動作のみ |
| AAA構造厳守 | Arrange→Act→Assert |
| 独立性確保 | SetUp/TearDownで毎回初期化 |

### バッチモードテスト実行

```powershell
# プロジェクトテストのみ
& "C:\Program Files\Unity\Hub\Editor\6000.3.2f1\Editor\Unity.exe" `
    -batchmode -runTests `
    -projectPath "D:\UnityProjects\kiban" `
    -testPlatform EditMode `
    -testResults "D:\UnityProjects\kiban\Logs\TestResults.xml" `
    -logFile "D:\UnityProjects\kiban\Logs\unity_test.log" `
    -assemblyNames "asterivo.kiban.Tests;asterivo.kiban.Tests.Core.Services;asterivo.kiban.Tests.Core.Editor;asterivo.kiban.Tests.Core.ECS;asterivo.kiban.Tests.Performance;asterivo.kiban.Tests.Integration;asterivo.kiban.Tests.Runtime;asterivo.kiban.Tests.Core.Editor.Setup"

# 結果確認
python "D:\UnityProjects\kiban\Scripts\parse_test_results.py" --verbose
```

**注意**: `-runTests`使用時は`-quit`を使用しない（テスト未実行で終了する）

| Exit Code | 意味 |
|-----------|------|
| 0 | テスト全件合格、またはテスト未実行 |
| 2 | テスト実行完了、一部失敗あり |
| 3 | テスト実行失敗（コンパイルエラー等） |

### CI環境判定

```csharp
private static bool IsRunningInCIEnvironment()
{
    var ci = Environment.GetEnvironmentVariable("CI");
    var gh = Environment.GetEnvironmentVariable("GITHUB_ACTIONS");
    return !string.IsNullOrEmpty(ci) || !string.IsNullOrEmpty(gh);
}

// CI環境でスキップ
[Test]
public void MyTest()
{
    if (IsRunningInCIEnvironment())
    {
        Assert.Ignore("CI環境でスキップ");
        return;
    }
    // テストロジック
}
```

詳細: [`Docs/UNIT_TESTING_GUIDELINES.md`](Docs/UNIT_TESTING_GUIDELINES.md)、[`Docs/03_Systems/Testing/PLAYMODE_TESTING_GUIDE.md`](Docs/03_Systems/Testing/PLAYMODE_TESTING_GUIDE.md)

## デバッグログ規約

**素の`UnityEngine.Debug`の直接使用は禁止**。Core層デバッグシステムを使用。

### 禁止事項
```csharp
// ❌ 禁止
Debug.Log("メッセージ");
Debug.LogWarning("警告");
Debug.LogError("エラー");
```

### 推奨: Core層デバッグシステム

#### 1. ProjectDebug（簡易ログ - 最も推奨）
```csharp
using asterivo.kiban.Core;

ProjectDebug.Log("アプリケーション開始");
ProjectDebug.LogWarning("メモリ使用量が増加");
ProjectDebug.LogError("リソースのロード失敗");
ProjectDebug.LogEditor("開発専用");  // リリースビルド除外
ProjectDebug.LogConditional("詳細", FeatureFlags.EnableVerboseLogging);
```

#### 2. IDebugService（高機能ログ）
```csharp
using asterivo.kiban.Core.Debug;
using asterivo.kiban.Core.Services;

var debug = ServiceLocator.Get<IDebugService>();

// カテゴリ付きログ
debug.LogInfo("ゲーム開始", "Game");
debug.LogWarning("FPS低下検出", "Performance");

// パフォーマンス計測
using (debug.BeginPerformanceSample("AI計算")) { /* 計測処理 */ }

// 診断とエクスポート
var report = debug.RunDiagnostics();
debug.ExportLogs("Logs/debug.json", LogExportFormat.Json);
```

#### 3. IEventLogger（イベント追跡）
```csharp
var logger = ServiceLocator.GetService<IEventLogger>();
logger.LogEvent("PlayerDamaged", listenerCount: 3, payload: "damage:25");
logger.ExportToCSV("Logs/events.csv");
```

### 使い分けガイド

| 用途 | 推奨システム |
|------|-------------|
| 簡易デバッグログ | `ProjectDebug.Log()` |
| Editor専用 | `ProjectDebug.LogEditor()` |
| カテゴリ分類 | `IDebugService.LogInfo(msg, category)` |
| 性能計測 | `IDebugService.BeginPerformanceSample()` |
| イベント追跡 | `IEventLogger.LogEvent()` |
| ログエクスポート | `IDebugService.ExportLogs()` |

## 注意事項

### 必須ルール
- **コーディング規約**: [`Docs/CODING_CONVENTIONS.md`](Docs/CODING_CONVENTIONS.md)
- **Core層設計**: [`Docs/CORE_INTERFACE_DESIGN_GUIDELINES.md`](Docs/CORE_INTERFACE_DESIGN_GUIDELINES.md)
- **テストガイドライン**: [`Docs/UNIT_TESTING_GUIDELINES.md`](Docs/UNIT_TESTING_GUIDELINES.md)
- **新機能は`Features`に追加**（Coreに混在させない）
- **テストは`Tests`に配置**
- **ドキュメントは`Docs/`に配置**
- **ログは`Logs/`に配置**（Assets外）
- **サードパーティ（`Assets/Plugins`）は直接編集禁止**

### 4層責務の遵守
- **ゲーム固有実装** → `Game`層 ✏️
- **機能部品** → `Feature`層 🔒
- **普遍的仕組み** → `Core`層 🔒
- **テンプレート** → `Template`層 📖（コピーして使用）
- **依存関係** → 一方向 (`Game` → `Template` → `Feature` → `Core`)

### ファイル命名規則
- **ソースコード/asmdef**: PascalCase（例: `PlayerController.cs`）
- **ソースコード、asmdef、metaファイルの名前は一致**させる
- **構文エラー/コンパイルエラーは必ず解消**してから次の作業へ

### 非同期処理
```csharp
// Unity 6 Awaitable使用（UniTaskは削除済み）
async Awaitable DoSomethingAsync()
{
    await Awaitable.WaitForSecondsAsync(1.0f);
    await Awaitable.NextFrameAsync();
}
```

### 入力処理
```csharp
// 旧Input Manager API禁止
var inputService = ServiceLocator.GetService<IInputService>();
if (inputService.JumpPressed) { /* ジャンプ */ }
Vector2 move = inputService.MoveInput;
```

詳細: [`Docs/InputSystem_Migration_TODO.md`](Docs/InputSystem_Migration_TODO.md)

### JSON操作規約（Newtonsoft Json必須）

**素の`UnityEngine.JsonUtility`の使用は禁止**。Newtonsoft Json（`Newtonsoft.Json`）を使用。

#### 禁止事項
```csharp
// ❌ 禁止 - Unity標準JsonUtility
var json = JsonUtility.ToJson(obj);
var obj = JsonUtility.FromJson<T>(json);
```

#### 推奨: Newtonsoft Json
```csharp
using Newtonsoft.Json;

// ✅ 推奨 - Newtonsoft Json
var json = JsonConvert.SerializeObject(obj, Formatting.Indented);
var obj = JsonConvert.DeserializeObject<T>(json);

// 設定カスタマイズ
var settings = new JsonSerializerSettings
{
    NullValueHandling = NullValueHandling.Ignore,
    Formatting = Formatting.Indented
};
var json = JsonConvert.SerializeObject(obj, settings);
```

#### Newtonsoft Jsonを使用する理由

| 機能 | JsonUtility | Newtonsoft Json |
|------|-------------|-----------------|
| Dictionary対応 | ❌ | ✅ |
| private フィールド | ❌ | ✅ |
| null処理 | ❌ | ✅ |
| カスタム変換 | ❌ | ✅ |
| LINQ to JSON | ❌ | ✅ |
| 複雑なネスト | △ | ✅ |

**パッケージ**: `com.unity.nuget.newtonsoft-json@3.2.1`（導入済み）

## 制約

- ⚠️ **【最重要】バッチモード起動前にUnity Editorを終了**
- **コンパイルエラーを解消してから実行**
- **プレイモード実行前にコンパイルエラーを全て解消**

## バッチコンパイル自動実行

以下の場合に自動実行:
1. 5つ以上のC#ファイル変更・作成
2. asmdef変更
3. 名前空間の大規模リファクタリング
4. Core層のアーキテクチャ変更
5. 複数コンパイルエラー修正後
6. ServiceLocatorへの新規サービス登録
7. GameEventの大量追加・変更
8. パッケージの追加・更新

**実行方法**:
```
mcp__ai-game-developer__manage_menu_item(
    action="execute",
    menu_path="Tools/Batch Compile/Save Compile and Restart"
)
```

**実行前確認**:
- Unity Editorプロセスの確認・終了（必須）
- すべての変更をファイルに保存済み

## テスト結果出力例

```
=== Unity Test Results ===
Total:    4413
Passed:   4367
Failed:   0
Skipped:  46
Duration: 55.87s

RESULT: PASSED
```




このプロジェクトを [Game Animation Sample](https://www.fab.com/ja/listings/880e319a-a59e-4ed2-b268-b32dac7fa016) へ統合したい。 その方法を詳細に検討して。
Game Animation Sample を元にしたプロジェクトは @"C:\Users\work0\uechi\UEProjects\AthleticTrial\" に、あります。


Game Animation Sample への統合の下準備しているので調査したのですが
**公式発表は無いですが**

UE6からUEFNスケルトンが標準になりそうな動きです。
Animation Blueprint の後継システムとして、UAF（Unreal Animation Framework）が開発中ですが
UE 5.8 では予定として、GASP(Game Animation Support Package) に UAF で完全に構築されたキャラクターを追加するようです。

┌─────────────────┐     ┌──────────────────────────────┐
│   Get Owner     │     │ Cast To CBP_SandboxCharacter │
│                 │     │                              │
│          ● ─────┼────→│ Object                       │
└─────────────────┘     │                              │
                        │         As CBP... ● ─────────┼──→ ┌─────────────────┐
                        └──────────────────────────────┘    │ Set ClimbInput  │
                                                            │                 │
                        ┌─────────────────┐                 │ ClimbInput ← ●  │
                        │ [移動入力値]     │                 │        (Vector2D)
                        │  (Vector2D)     │                 │                 │
                        │          ● ─────┼────────────────→│                 │
                        └─────────────────┘                 └─────────────────┘

# 現在の修正状況
壁吸着を Fキー 、 壁離脱を Gキー と変更して、IMC_Climbing を修正。
ですが、壁検出用 Trace Line が発射されず、壁吸着もできない。
そこで、 IMC_Sandbox に Fキー マッピングを追加すると、Trace Line は発射されるようになったが、
Trace Line の発射方向がプレイヤーキャラの正面ではなく斜めに発射されている状況

# 問題１の回答
- MC_Sandbox に追加した F キーは IA_GrabWall にマッピングした。

Fキー を入力するとカメラもキャラクターも動かなくなる。よってTrace Line の発射方向が確認できない。
更に Fキー を連続入力するとキャラクターが、どんどん地面に潜っていき、最終的には腰の位置まで地面に潜る **Ultra Think**


## 確認
F キーを押したとき、画面に以下のデバッグメッセージ

- "CheckForClimbableSurface Called" (青色): 表示される
- "HitWall: true/false" (青色): true
- "Condition Result: true/false" (青色): true

## 原因
1. 1つ
2. カメラを動かせないため確認できない
**Ultra Think**


- Impact Normal: X=0.000 Y=1.000 Z=0.000
- WallOffset: 60.0
**Ultra Think**


## EnableWallClimbingTick
[N0]EnableWallClimbingTick{O:then>N2.execute} [N1]Self-Reference{O:self>N2.self} [N2]SetComponentTickEnabled{I:execute<N0.then,self<N1.self;O:then>N3.execute;D:bEnabled=true} [N3]SetbIsWallClimbingActive{I:execute<N2.then;O:then>N4.execute;D:bIsWallClimbingActive=true} [N4]SetbIsClimbing{I:execute<N3.then;D:bIsClimbing=true}

## EnableWallClimbingTick 
[N0]DisableAllClimbingTicks{O:then>N2.execute} [N1]Self-Reference{O:self>N2.self} [N2]SetComponentTickEnabled{I:execute<N0.then,self<N1.self;O:then>N3.execute;D:bEnabled=false} [N3]SetbIsWallClimbingActive{I:execute<N2.then;O:then>N4.execute;D:bIsWallClimbingActive=false} [N4]SetbIsLedgeClimbingActive{I:execute<N3.then;O:then>N5.execute;D:bIsLedgeClimbingActive=false} [N5]SetbIsClimbing{I:execute<N4.then;D:bIsClimbing=false}

これでよいですか？ **Ultra Think**



- 1回目の F キー: 壁に吸着しません
- 2回目の F キー: 壁離脱は Gキー
- 連続で F キーを押しても 地面に潜らない: 地面に潜らなくなりました。カメラ操作もキャラ操作も可能になりました。
- 1回目の F キー のときのスクリーション と 2回目の F キー のときのスクリーション

**ログの読み方は上から下へ**
## 1回目の F キー
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: false
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] GrabWall Triggered
LogBlueprintUserMessages: [ClimbingComponent] P=7.175000 Y=273.549998 R=0.000000
LogBlueprintUserMessages: [ClimbingComponent] CheckForClimbableSurface Called
LogBlueprintUserMessages: [ClimbingComponent] X=0.000 Y=1.000 Z=0.000
LogBlueprintUserMessages: [ClimbingComponent] HitWall: true
LogBlueprintUserMessages: [ClimbingComponent] Condition Result: true
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] StartClimbing_GASP - Active
LogBlueprintUserMessages: [ClimbingComponent] EnableWallClimbingTick Called
LogBlueprintUserMessages: [ClimbingComponent] F Key Pressed
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] StopClimbing_GASP - Deactivated
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: false

## 2回目の F キー
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: false
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] GrabWall Triggered
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] StopClimbing_GASP - Deactivated
LogBlueprintUserMessages: [ClimbingComponent] F Key Pressed
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] StopClimbing_GASP - Deactivated
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: false


**ログの読み方は上から下へ** **Ultra Think**
## 1回目の F キー
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: false
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] GrabWall Triggered
LogBlueprintUserMessages: [ClimbingComponent] P=1.750000 Y=262.349999 R=0.000000
LogBlueprintUserMessages: [ClimbingComponent] CheckForClimbableSurface Called
LogBlueprintUserMessages: [ClimbingComponent] X=0.000 Y=1.000 Z=0.000
LogBlueprintUserMessages: [ClimbingComponent] HitWall: true
LogBlueprintUserMessages: [ClimbingComponent] Condition Result: true
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] StartClimbing_GASP - Active
LogBlueprintUserMessages: [ClimbingComponent] EnableWallClimbingTick Called
LogBlueprintUserMessages: [ClimbingComponent] F Key Pressed
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] StopClimbing_GASP - Deactivated
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: false

**ログの読み方は上から下へ** **Ultra Think**
## AC_Climbing 内に 「F Key Pressed」の PrintString があるのは、ここだけ **Ultra Think**
[N0]PrintString{D:bPrintToLog=true,bPrintToScreen=true,Duration=2.0,InString=FKeyPressed,Key=None,self=/Script/Engine.Default__KismetSystemLibrary,TextColor=(R=0.000000,G=0.660000,B=1.000000,A=1.000000)}



**ログの読み方は上から下へ** **Ultra Think**
## 1回目の F キー
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: false
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] GrabWall Triggered
LogBlueprintUserMessages: [ClimbingComponent] P=5.425000 Y=264.099999 R=0.000000
LogBlueprintUserMessages: [ClimbingComponent] CheckForClimbableSurface Called
LogBlueprintUserMessages: [ClimbingComponent] X=0.000 Y=1.000 Z=0.000
LogBlueprintUserMessages: [ClimbingComponent] HitWall: true
LogBlueprintUserMessages: [ClimbingComponent] Condition Result: true
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] StartClimbing_GASP - Active
LogBlueprintUserMessages: [ClimbingComponent] EnableWallClimbingTick Called
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=-3.415 Y=-820.000 Z=91.149
LogBlueprintUserMessages: [ClimbingComponent] No input - skipping Line Trace
LogBlueprintUserMessages: [ClimbingComponent] UpdateWallAttachment Called
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=-3.415 Y=-820.000 Z=91.149
LogBlueprintUserMessages: [ClimbingComponent] No input - skipping Line Trace
LogBlueprintUserMessages: [ClimbingComponent] UpdateWallAttachment Called
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=-3.415 Y=-820.000 Z=91.149
LogBlueprintUserMessages: [ClimbingComponent] No input - skipping Line Trace

## UpdateWallAttachment 関数の内容  **Ultra Think**
[N0]UpdateWallAttachment{O:DeltaTime>N10.DeltaTime,DeltaTime>N9.DeltaTime,then>N8.execute} [N1]GetOwnerCharacter{O:OwnerCharacter>N11.self,OwnerCharacter>N12.self,OwnerCharacter>N2.self,OwnerCharacter>N3.self} [N2]GetActorRotation{I:self<N1.OwnerCharacter;O:ReturnValue>N9.Current} [N3]GetActorLocation{I:self<N1.OwnerCharacter;O:ReturnValue>N10.Current} [N4]GetTargetWallRotation{O:TargetWallRotation>N9.Target} [N5]GetRotationInterpSpeed{O:RotationInterpSpeed>N9.InterpSpeed} [N6]GetWallHitLocation{O:WallHitLocation>N10.Target} [N7]GetLocationInterpSpeed{O:LocationInterpSpeed>N10.InterpSpeed} [N8]PrintString{I:execute<N0.then;O:then>N11.execute;D:bPrintToLog=true,bPrintToScreen=true,Duration=2.0,InString=UpdateWallAttachmentCalled,Key=None,self=/Script/Engine.Default__KismetSystemLibrary,TextColor=(R=0.000000,G=0.660000,B=1.000000,A=1.000000)} [N9]RInterpTo{I:Current<N2.ReturnValue,DeltaTime<N0.DeltaTime,InterpSpeed<N5.RotationInterpSpeed,Target<N4.TargetWallRotation;O:ReturnValue>N11.NewRotation;D:Current=0,0,0,DeltaTime=0.0,InterpSpeed=0.0,self=/Script/Engine.Default__KismetMathLibrary,Target=0,0,0} [N10]VInterpTo{I:Current<N3.ReturnValue,DeltaTime<N0.DeltaTime,InterpSpeed<N7.LocationInterpSpeed,Target<N6.WallHitLocation;O:ReturnValue>N12.NewLocation;D:Current=0,0,0,DeltaTime=0.0,InterpSpeed=0.0,self=/Script/Engine.Default__KismetMathLibrary,Target=0,0,0} [N11]SetActorRotation{I:execute<N8.then,NewRotation<N9.ReturnValue,self<N1.OwnerCharacter;O:then>N12.execute;D:bTeleportPhysics=false,NewRotation=0,0,0} [N12]SetActorLocation{I:execute<N11.then,NewLocation<N10.ReturnValue,self<N1.OwnerCharacter;D:bSweep=false,bTeleport=false,NewLocation=0,0,0}


## TryGrabWall 関数の内容  **Ultra Think**
[N0]IsFalling{I:self<N1.CharacterMovement;O:ReturnValue>N5.A} [N1]GetCharacterMovement{I:self<N2.OwnerCharacter;O:CharacterMovement>N0.self} [N2]GetOwnerCharacter{O:OwnerCharacter>N1.self} [N3]GetbIsClimbing{O:bIsClimbing>N4.A} [N4]NOTBoolean{I:A<N3.bIsClimbing;O:ReturnValue>N8.A;D:A=false,self=/Script/Engine.Default__KismetMathLibrary} [N5]NOTBoolean{I:A<N0.ReturnValue;O:ReturnValue>N8.B;D:A=false,self=/Script/Engine.Default__KismetMathLibrary} [N6]TryGrabWall{O:then>N7.execute} [N7]CheckForClimbableSurface{I:execute<N6.then;O:bHitWall>N10.B,bHitWall>N9.InBool,HitResult>N18.HitResult,then>N14.execute} [N8]ANDBoolean{I:A<N4.ReturnValue,B<N5.ReturnValue;O:ReturnValue>N10.A;D:A=false,B=false,self=/Script/Engine.Default__KismetMathLibrary} [N9]ToString(Boolean){I:InBool<N7.bHitWall;O:ReturnValue>N11.B;D:InBool=false,self=/Script/Engine.Default__KismetStringLibrary} [N10]ANDBoolean{I:A<N8.ReturnValue,B<N7.bHitWall;O:ReturnValue>N12.InBool,ReturnValue>N16.Condition;D:A=false,B=false,self=/Script/Engine.Default__KismetMathLibrary} [N11]Append{I:B<N9.ReturnValue;O:ReturnValue>N14.InString;D:A=HitWall:,self=/Script/Engine.Default__KismetStringLibrary} [N12]ToString(Boolean){I:InBool<N10.ReturnValue;O:ReturnValue>N13.B;D:InBool=false,self=/Script/Engine.Default__KismetStringLibrary} [N13]Append{I:B<N12.ReturnValue;O:ReturnValue>N15.InString;D:A=ConditionResult:,self=/Script/Engine.Default__KismetStringLibrary} [N14]PrintString{I:execute<N7.then,InString<N11.ReturnValue;O:then>N15.execute;D:bPrintToLog=true,bPrintToScreen=true,Duration=2.0,InString=Hello,Key=None,self=/Script/Engine.Default__KismetSystemLibrary,TextColor=(R=0.000000,G=0.660000,B=1.000000,A=1.000000)} [N15]PrintString{I:execute<N14.then,InString<N13.ReturnValue;O:then>N16.execute;D:bPrintToLog=true,bPrintToScreen=true,Duration=2.0,InString=Hello,Key=None,self=/Script/Engine.Default__KismetSystemLibrary,TextColor=(R=0.000000,G=0.660000,B=1.000000,A=1.000000)} [N16]Branch{I:Condition<N10.ReturnValue,execute<N15.then;O:else>N17.execute,then>N18.execute;D:Condition=true} [N17]ReturnNode{I:execute<N16.else;D:Success=false} [N18]AttachToWall{I:execute<N16.then,HitResult<N7.HitResult;O:then>N19.execute} [N19]CallOnClimbingStarted{I:execute<N18.then;O:then>N20.execute} [N20]EnableWallClimbingTick{I:execute<N19.then;O:then>N21.execute} [N21]ReturnNode{I:execute<N20.then;D:Success=true}


[N0]IsFalling{I:self<N1.CharacterMovement;O:ReturnValue>N6.A} [N1]GetCharacterMovement{I:self<N2.OwnerCharacter;O:CharacterMovement>N0.self} [N2]GetOwnerCharacter{O:OwnerCharacter>N1.self} [N3]GetbIsClimbing{O:bIsClimbing>N4.A} [N4]NOTBoolean{I:A<N3.bIsClimbing;O:ReturnValue>N8.A;D:A=false,self=/Script/Engine.Default__KismetMathLibrary} [N5]TryGrabWall{O:then>N7.execute} [N6]NOTBoolean{I:A<N0.ReturnValue;O:ReturnValue>N8.B;D:A=false,self=/Script/Engine.Default__KismetMathLibrary} [N7]CheckForClimbableSurface{I:execute<N5.then;O:bHitWall>N10.B,bHitWall>N9.InBool,HitResult>N18.HitResult,then>N14.execute} [N8]ANDBoolean{I:A<N4.ReturnValue,B<N6.ReturnValue;O:ReturnValue>N10.A;D:A=false,B=false,self=/Script/Engine.Default__KismetMathLibrary} [N9]ToString(Boolean){I:InBool<N7.bHitWall;O:ReturnValue>N11.B;D:InBool=false,self=/Script/Engine.Default__KismetStringLibrary} [N10]ANDBoolean{I:A<N8.ReturnValue,B<N7.bHitWall;O:ReturnValue>N12.InBool,ReturnValue>N16.Condition;D:A=false,B=false,self=/Script/Engine.Default__KismetMathLibrary} [N11]Append{I:B<N9.ReturnValue;O:ReturnValue>N14.InString;D:A=HitWall:,self=/Script/Engine.Default__KismetStringLibrary} [N12]ToString(Boolean){I:InBool<N10.ReturnValue;O:ReturnValue>N13.B;D:InBool=false,self=/Script/Engine.Default__KismetStringLibrary} [N13]Append{I:B<N12.ReturnValue;O:ReturnValue>N15.InString;D:A=ConditionResult:,self=/Script/Engine.Default__KismetStringLibrary} [N14]PrintString{I:execute<N7.then,InString<N11.ReturnValue;O:then>N15.execute;D:bPrintToLog=true,bPrintToScreen=true,Duration=2.0,InString=Hello,Key=None,self=/Script/Engine.Default__KismetSystemLibrary,TextColor=(R=0.000000,G=0.660000,B=1.000000,A=1.000000)} [N15]PrintString{I:execute<N14.then,InString<N13.ReturnValue;O:then>N16.execute;D:bPrintToLog=true,bPrintToScreen=true,Duration=2.0,InString=Hello,Key=None,self=/Script/Engine.Default__KismetSystemLibrary,TextColor=(R=0.000000,G=0.660000,B=1.000000,A=1.000000)} [N16]Branch{I:Condition<N10.ReturnValue,execute<N15.then;O:else>N17.execute,then>N18.execute;D:Condition=true} [N17]ReturnNode{I:execute<N16.else;D:Success=false} [N18]AttachToWall{I:execute<N16.then,HitResult<N7.HitResult;O:then>N21.execute} [N19]GetCharacterMovement{I:self<N20.OwnerCharacter;O:CharacterMovement>N21.self} [N20]GetOwnerCharacter{O:OwnerCharacter>N19.self} [N21]SetMovementMode{I:execute<N18.then,self<N19.CharacterMovement;O:then>N22.execute;D:NewCustomMode=0.0,NewMovementMode=MOVE_Flying} [N22]CallOnClimbingStarted{I:execute<N21.then;O:then>N23.execute} [N23]EnableWallClimbingTick{I:execute<N22.then;O:then>N24.execute} [N24]ReturnNode{I:execute<N23.then;D:Success=true}
これで正しいですか？ **Ultra Think**

LocationInterpSpeed 変数のデフォルト値: 100.0
RotationInterpSpeed 変数のデフォルト値: 100.0


**ログの読み方は上から下へ** **Ultra Think**
## 1回目の F キー
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: false
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] GrabWall Triggered
LogBlueprintUserMessages: [ClimbingComponent] P=5.250000 Y=274.949999 R=0.000000
LogBlueprintUserMessages: [ClimbingComponent] CheckForClimbableSurface Called
LogBlueprintUserMessages: [ClimbingComponent] X=0.000 Y=1.000 Z=0.000
LogBlueprintUserMessages: [ClimbingComponent] HitWall: true
LogBlueprintUserMessages: [ClimbingComponent] Condition Result: true
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] StartClimbing_GASP - Active
LogBlueprintUserMessages: [ClimbingComponent] EnableWallClimbingTick Called
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=27.918 Y=-820.000 Z=92.541
LogBlueprintUserMessages: [ClimbingComponent] No input - skipping Line Trace
LogBlueprintUserMessages: [ClimbingComponent] Flying
LogBlueprintUserMessages: [ClimbingComponent] UpdateWallAttachment Called
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=27.918 Y=-820.000 Z=92.541
LogBlueprintUserMessages: [ClimbingComponent] No input - skipping Line Trace
LogBlueprintUserMessages: [ClimbingComponent] Flying
LogBlueprintUserMessages: [ClimbingComponent] UpdateWallAttachment Called
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=27.918 Y=-820.000 Z=92.541
LogBlueprintUserMessages: [ClimbingComponent] No input - skipping Line Trace
LogBlueprintUserMessages: [ClimbingComponent] Flying
LogBlueprintUserMessages: [ClimbingComponent] UpdateWallAttachment Called
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=27.918 Y=-820.000 Z=92.541
LogBlueprintUserMessages: [ClimbingComponent] No input - skipping Line Trace
LogBlueprintUserMessages: [ClimbingComponent] Flying
LogBlueprintUserMessages: [ClimbingComponent] UpdateWallAttachment Called
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=27.918 Y=-820.000 Z=92.541
LogBlueprintUserMessages: [ClimbingComponent] No input - skipping Line Trace
LogBlueprintUserMessages: [ClimbingComponent] Flying
LogBlueprintUserMessages: [ClimbingComponent] UpdateWallAttachment Called


**ログの読み方は上から下へ** **Ultra Think**
## 1回目の F キー
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: false
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] GrabWall Triggered
LogBlueprintUserMessages: [ClimbingComponent] P=355.275000 Y=260.249999 R=0.000000
LogBlueprintUserMessages: [ClimbingComponent] CheckForClimbableSurface Called
LogBlueprintUserMessages: [ClimbingComponent] X=0.000 Y=1.000 Z=0.000
LogBlueprintUserMessages: [ClimbingComponent] HitWall: true
LogBlueprintUserMessages: [ClimbingComponent] Condition Result: true
LogBlueprintUserMessages: [CBP_SandboxCharacter_C_0] StartClimbing_GASP - Active
LogBlueprintUserMessages: [ClimbingComponent] EnableWallClimbingTick Called
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=-64.119 Y=-820.000 Z=84.671
LogBlueprintUserMessages: [ClimbingComponent] No input - skipping Line Trace
LogBlueprintUserMessages: [ClimbingComponent] Flying
LogBlueprintUserMessages: [ClimbingComponent] UpdateWallAttachment Called
LogBlueprintUserMessages: [ClimbingComponent] CurrentLoc: X=-56.735 Y=-837.024 Z=88.275
LogBlueprintUserMessages: [ClimbingComponent] NewLoc: X=-64.064 Y=-820.127 Z=84.698
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=-64.119 Y=-820.000 Z=84.671
LogBlueprintUserMessages: [ClimbingComponent] No input - skipping Line Trace
LogBlueprintUserMessages: [ClimbingComponent] Flying
LogBlueprintUserMessages: [ClimbingComponent] UpdateWallAttachment Called
LogBlueprintUserMessages: [ClimbingComponent] CurrentLoc: X=-64.064 Y=-820.127 Z=84.698
LogBlueprintUserMessages: [ClimbingComponent] NewLoc: X=-64.119 Y=-820.000 Z=84.671
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=-64.119 Y=-820.000 Z=84.671
LogBlueprintUserMessages: [ClimbingComponent] No input - skipping Line Trace
LogBlueprintUserMessages: [ClimbingComponent] Flying
LogBlueprintUserMessages: [ClimbingComponent] UpdateWallAttachment Called
LogBlueprintUserMessages: [ClimbingComponent] CurrentLoc: X=-64.119 Y=-820.000 Z=84.671
LogBlueprintUserMessages: [ClimbingComponent] NewLoc: X=-64.119 Y=-820.000 Z=84.671
LogBlueprintUserMessages: [ClimbingComponent] AC_Climbing Tick - bIsWallClimbingActive: true
LogBlueprintUserMessages: [ClimbingComponent] WallHitLocation: X=-64.119 Y=-820.000 Z=84.671

## AttachToWall 関数の内容
[N0]AttachToWall{O:HitResult>N1.Hit,HitResult>N6.Hit,then>N2.execute} [N1]BreakHitResult{I:Hit<N0.HitResult;O:ImpactNormal>N2.WallNormal,ImpactNormal>N3.A;D:self=/Script/Engine.Default__GameplayStatics} [N2]SetWallNormal{I:execute<N0.then,WallNormal<N1.ImpactNormal;O:then>N5.execute;D:WallNormal=0,0,0} [N3]vector*vector{I:A<N1.ImpactNormal;O:ReturnValue>N4.X;D:B=-1.0} [N4]MakeRotfromX{I:X<N3.ReturnValue;O:ReturnValue>N5.TargetWallRotation;D:self=/Script/Engine.Default__KismetMathLibrary,X=0,0,0} [N5]SetTargetWallRotation{I:execute<N2.then,TargetWallRotation<N4.ReturnValue;O:then>N10.execute;D:TargetWallRotation=0,0,0} [N6]BreakHitResult{I:Hit<N0.HitResult;O:ImpactPoint>N11.A;D:self=/Script/Engine.Default__GameplayStatics} [N7]GetWallNormal{O:WallNormal>N9.A} [N8]GetWallOffset{O:WallOffset>N9.B} [N9]vector*vector{I:A<N7.WallNormal,B<N8.WallOffset;O:ReturnValue>N11.B} [N10]SetWallHitLocation{I:execute<N5.then,WallHitLocation<N11.ReturnValue;O:then>N12.execute;D:WallHitLocation=0,0,0} [N11]vector+vector{I:A<N6.ImpactPoint,B<N9.ReturnValue;O:ReturnValue>N10.WallHitLocation} [N12]ReturnNode{I:execute<N10.then;D:Success=true}


 WallOffset 変数のデフォルト値: 60.0


## CheckForClimbableSurface 関数の内容
 [N0]CheckForClimbableSurface{O:then>N2.execute} [N1]GetOwnerCharacter{O:OwnerCharacter>N6.self,OwnerCharacter>N7.self} [N2]PrintString{I:execute<N0.then,InString<N3.ReturnValue;O:then>N8.execute;D:bPrintToLog=true,bPrintToScreen=true,Duration=2.0,InString=Hello,Key=None,self=/Script/Engine.Default__KismetSystemLibrary,TextColor=(R=0.000000,G=0.660000,B=1.000000,A=1.000000)} [N3]ToString(Rotator){I:InRot<N5.ReturnValue;O:ReturnValue>N2.InString;D:InRot=0,0,0,self=/Script/Engine.Default__KismetStringLibrary} [N4]GetForwardVector{I:InRot<N5.ReturnValue;O:ReturnValue>N10.A;D:InRot=0,0,0,self=/Script/Engine.Default__KismetMathLibrary} [N5]GetControlRotation{I:self<N6.ReturnValue;O:ReturnValue>N3.InRot,ReturnValue>N4.InRot} [N6]GetController{I:self<N1.OwnerCharacter;O:ReturnValue>N5.self} [N7]GetActorLocation{I:self<N1.OwnerCharacter;O:ReturnValue>N11.A,ReturnValue>N12.Start} [N8]PrintString{I:execute<N2.then;O:then>N12.execute;D:bPrintToLog=true,bPrintToScreen=true,Duration=2.0,InString=CheckForClimbableSurfaceCalled,Key=None,self=/Script/Engine.Default__KismetSystemLibrary,TextColor=(R=0.000000,G=0.660000,B=1.000000,A=1.000000)} [N9]GetTraceLength{O:TraceLength>N10.B} [N10]vector*vector{I:A<N4.ReturnValue,B<N9.TraceLength;O:ReturnValue>N11.B} [N11]vector+vector{I:A<N7.ReturnValue,B<N10.ReturnValue;O:ReturnValue>N12.End} [N12]LineTraceByChannel{I:End<N11.ReturnValue,execute<N8.then,Start<N7.ReturnValue;O:OutHit>N13.Hit,OutHit>N15.HitResult,OutHit>N25.HitResult,ReturnValue>N14.Condition,then>N14.execute;D:bIgnoreSelf=true,bTraceComplex=false,DrawDebugType=ForDuration,DrawTime=10.0,End=0,0,0,self=/Script/Engine.Default__KismetSystemLibrary,Start=0,0,0,TraceChannel=TraceTypeQuery1,TraceColor=(R=1.000000,G=0.000000,B=0.000000,A=1.000000),TraceHitColor=(R=0.000000,G=1.000000,B=0.000000,A=1.000000)} [N13]BreakHitResult{I:Hit<N12.OutHit;O:ImpactNormal>N16.InVec,ImpactNormal>N19.InVec,ImpactNormal>N23.WallNormal,ImpactPoint>N24.WallHitLocation;D:self=/Script/Engine.Default__GameplayStatics} [N14]Branch{I:Condition<N12.ReturnValue,execute<N12.then;O:else>N15.execute,then>N17.execute;D:Condition=true} [N15]ReturnNode{I:execute<N14.else,HitResult<N12.OutHit;D:bHitWall=false} [N16]BreakVector{I:InVec<N13.ImpactNormal;O:Z>N18.A;D:InVec=0,0,0,self=/Script/Engine.Default__KismetMathLibrary} [N17]Branch{I:Condition<N20.ReturnValue,execute<N14.then;O:else>N21.execute,then>N22.execute;D:Condition=true} [N18]Absolute(Float){I:A<N16.Z;O:ReturnValue>N20.A;D:A=0.0,self=/Script/Engine.Default__KismetMathLibrary} [N19]ToString(Vector){I:InVec<N13.ImpactNormal;O:ReturnValue>N22.InString;D:InVec=0,0,0,self=/Script/Engine.Default__KismetStringLibrary} [N20]float<float{I:A<N18.ReturnValue;O:ReturnValue>N17.Condition;D:B=0.1} [N21]ReturnNode{I:execute<N17.else;D:bHitWall=false} [N22]PrintString{I:execute<N17.then,InString<N19.ReturnValue;O:then>N23.execute;D:bPrintToLog=true,bPrintToScreen=true,Duration=2.0,InString=Hello,Key=None,self=/Script/Engine.Default__KismetSystemLibrary,TextColor=(R=0.000000,G=0.660000,B=1.000000,A=1.000000)} [N23]SetWallNormal{I:execute<N22.then,WallNormal<N13.ImpactNormal;O:then>N24.execute;D:WallNormal=0,0,0} [N24]SetWallHitLocation{I:execute<N23.then,WallHitLocation<N13.ImpactPoint;O:then>N25.execute;D:WallHitLocation=0,0,0} [N25]ReturnNode{I:execute<N24.then,HitResult<N12.OutHit;D:bHitWall=true}




壁頂上に立つために、キャラの頭から数十センチ上にオフセットした位置から壁に向けて数十センチ先をEnd位置として Trace Line を発射しています。壁吸着時に、その Trace Line が壁を見失ったら頂上に到達したとみなして、Eキー で頂上に登って立つ仕様で実装も存在します。ですが、Eキー は GASP と競合しているので、別の キー にマッピングし直す必要があります。 Gキーでも良いでしょう。 以上を検討して **Ultra Think**
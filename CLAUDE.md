...
このプロジェクトは **SDD（Spec-Driven Development / 仕様駆動開発）** を採用しています。

### SDD とは
- 仕様を「**唯一の真実源泉（Single Source of Truth）**」として位置づける開発手法
- 仕様からコード・テスト・ドキュメントを生成・更新する
- AI エージェントとの協業を前提に、意図したコードを書いてもらうためのガードレール

### SDD ワークフロー
```
1. 仕様作成（Specification）
   └─ SPEC.md に機能仕様、受け入れ基準を定義
...
4. 実装と検証（Implementation & Verification）
   └─ 仕様に基づいてコード生成、テスト
```

### 仕様書
- **[SPEC.md](SPEC.md)** - プロジェクト仕様書（唯一の真実源泉）
  - 機能仕様
...
### SDD の原則
1. **仕様ファースト**: コードを書く前に SPEC.md を更新する
2. **仕様との整合性**: 実装は常に SPEC.md の記述に従う
3. **変更履歴**: 仕様変更時は SPEC.md の変更履歴を更新する
4. **受け入れ基準**: 機能追加時は Given-When-Then 形式で受け入れ基準を定義する
